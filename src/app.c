...
void sendPollRequest(sl_cli_command_arg_t *arguments);

static const sl_cli_command_info_t cmd_sendPollRequest = \
    SL_CLI_COMMAND(sendPollRequest,
                   "send poll request to parent",
                   "",
                   {SL_CLI_ARG_END, });

// Create the array of commands, containing three elements in this example
static sl_cli_command_entry_t a_table[] = {
    {"sendPollRequest", &cmd_sendPollRequest, false },
    {NULL, NULL, false },
};
...
// CLI for manual poll request
void sendPollRequest(sl_cli_command_arg_t *arguments) {
  EmberStatus status;
  status = emberPollForData();
  sl_zigbee_app_debug_println("emberPollForData, status: 0x%02x",
                                status);
}

// Poll completed callback (enable callback via end-device-support plugin)
void emberAfPluginEndDeviceSupportPollCompletedCallback(EmberStatus status) {

  sl_zigbee_app_debug_println("Poll complete, status: 0x%02X",
                                status);
